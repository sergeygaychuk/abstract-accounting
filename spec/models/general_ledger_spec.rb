...
describe GeneralLedger do
  ...
  describe "#current_scope" do
    it 'should' do
      GeneralLedger.all.should eq(Txn.without_nulable.all)
    end
  ...
  describe "#by_deal" do
    it "should filter txns" do
      ...
      GeneralLedger.by_deal(wb.deal_id).all.should =~ Txn.without_nulable.all[3, 1]
    end
  end
  ...
  describe "#app_utils" do
    ...
    it "should proxy filtrate method to Txn" do
      GeneralLedger.filtrate(sort: { field: "resource", type: "desc" }).
          all.should eq(Txn.without_nulable.sort_by_resource("desc"))
    end
  ...
  describe "#by_deals" do
    it "should filter txns" do
      ...
      GeneralLedger.by_deals(deal_ids).all.should eq(Txn.without_nulable.joins{fact}.
