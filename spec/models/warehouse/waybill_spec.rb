...
describe Warehouse::Waybill do
  ...
  it 'should sort waybills' do
    ...
    wbs = Warehouse::Waybill.order_by(field: 'created', type: 'asc').all
    wbs_test = Warehouse::Waybill.order('created').all
    wbs.should eq(wbs_test)
    wbs = Warehouse::Waybill.order_by(field: 'created', type: 'desc').all
    wbs_test = Warehouse::Waybill.order('created DESC').all
    wbs.should eq(wbs_test)

    wbs = Warehouse::Waybill.order_by(field: 'document_id', type: 'asc').all
    wbs_test = Warehouse::Waybill.order('document_id').all
    wbs.should eq(wbs_test)
    wbs = Warehouse::Waybill.order_by(field: 'document_id', type: 'desc').all
    wbs_test = Warehouse::Waybill.order('document_id DESC').all
    wbs.should eq(wbs_test)

    wbs = Warehouse::Waybill.order_by(field: 'distributor', type: 'asc').all
    wbs_test = Warehouse::Waybill.joins{deal.rules.from.entity(LegalEntity).outer}.
        ...
        group('waybills.id, waybills.created, waybills.document_id, waybills.deal_id,' +
                  'froms_rules.entity_type, entities.tag, legal_entities.name').
        order("case froms_rules.entity_type
                      when 'Entity'      then entities.tag
                      when 'LegalEntity' then legal_entities.name
                 end").all
    wbs.should eq(wbs_test)
    wbs = Warehouse::Waybill.order_by(field: 'distributor', type: 'desc').all
    wbs_test = Warehouse::Waybill.joins{deal.rules.from.entity(LegalEntity).outer}.
        joins{deal.rules.from.entity(Entity).outer}.
        group('waybills.id, waybills.created, waybills.document_id, waybills.deal_id,' +
                  'froms_rules.entity_type, entities.tag, legal_entities.name').
    ...
    wbs = Warehouse::Waybill.order_by(field: 'storekeeper', type: 'asc').all
    wbs_test = Warehouse::Waybill.joins{deal.entity(Entity)}.order('entities.tag').all
    wbs.should eq(wbs_test)
    wbs = Warehouse::Waybill.order_by(field: 'storekeeper', type: 'desc').all
    wbs_test = Warehouse::Waybill.joins{deal.entity(Entity)}.order('entities.tag DESC').all
    wbs.should eq(wbs_test)

    wbs = Warehouse::Waybill.order_by(field: 'storekeeper_place', type: 'asc').all
    wbs_test = Warehouse::Waybill.joins{deal.take.place}.order('places.tag').all
    wbs.should eq(wbs_test)
    wbs = Warehouse::Waybill.order_by(field: 'storekeeper_place', type: 'desc').all
    wbs_test = Warehouse::Waybill.joins{deal.take.place}.order('places.tag DESC').all
    wbs.should eq(wbs_test)

    wbs = Warehouse::Waybill.order_by(field: 'sum', type: 'asc').all
    wbs_test = Warehouse::Waybill.joins{deal.rules.from}.
        ...
        order("sum").all
    wbs.should eq(wbs_test)
    wbs = Warehouse::Waybill.order_by(field: 'sum', type: 'desc').all
    wbs_test = Warehouse::Waybill.joins{deal.rules.from}.
        group('waybills.id, waybills.created, waybills.document_id, waybills.deal_id').
        select("waybills.*").
        select{sum(deal.rules.rate / deal.rules.from.rate).as(:sum)}.
  ...
  it 'should filter waybills' do
    ...
    Warehouse::Waybill.search({ 'state' => Warehouse::Waybill::APPLIED }).include?(wb).should be_true
    Warehouse::Waybill.search({ 'state' => Warehouse::Waybill::APPLIED }).include?(wb2).should be_false
    Warehouse::Waybill.search({ 'state' => Warehouse::Waybill::INWORK }).include?(wb).should be_false
    Warehouse::Waybill.search({ 'state' => Warehouse::Waybill::INWORK }).include?(wb2).should be_true
    Warehouse::Waybill.search({ 'state' => Warehouse::Waybill::CANCELED }).include?(wb).should be_false
    Warehouse::Waybill.search({ 'state' => Warehouse::Waybill::CANCELED }).include?(wb2).should be_false
    Warehouse::Waybill.search({ 'state' => Warehouse::Waybill::REVERSED }).include?(wb).should be_false
