...
describe Warehouse::Allocation do
  ...
  it 'should sort allocations' do
    ...
    als.should eq(als_test)
    als = Warehouse::Allocation.sort(field: 'foreman', type: 'desc').all
    als_test = Warehouse::Allocation.joins{deal.rules.to.entity(Entity)}.
        group('allocations.id, allocations.created, allocations.deal_id, entities.tag').
        order('entities.tag DESC').all
    als.should eq(als_test)

    als = Warehouse::Allocation.sort(field: 'state', type: 'asc').all
    als_test = Warehouse::Allocation.joins{deal.deal_state}.order('deal_states.state').all
    als.should eq(als_test)
    als = Warehouse::Allocation.sort(field: 'state', type: 'desc').all
    als_test = Warehouse::Allocation.joins{deal.deal_state}.order('deal_states.state DESC').all
    als.should eq(als_test)
  end
  ...
  it 'should search allocations' do
    ...
    Warehouse::Allocation.search({states: [Warehouse::Allocation::INWORK]}).should =~ Warehouse::Allocation.
