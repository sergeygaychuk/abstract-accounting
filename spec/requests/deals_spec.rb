...
feature 'deals', %q{
  ...
  scenario 'go to general_ledger page from deals', js: true do
    ...
    current_hash.should eq("general_ledger?deal_id=#{shipment_deal.id}&date=#{date}")
  ...
  scenario 'go to transcript page from deals', js: true, focus: true do
    rub = Chart.first.currency
    per_page = Settings.root.per_page
    x = create(:asset)
    purchase_x = create(:deal,
