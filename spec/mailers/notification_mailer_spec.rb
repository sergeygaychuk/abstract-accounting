...
describe NotificationMailer do
  describe "user_notification_email" do
    ...
    it "send to user notification email" do
      mail = NotificationMailer.user_notification_email(@user, @warning).deliver
    ...
    it "send notification email to all user's managers" do
      m1 = create(:user)
      m2 = create(:user)
      m3 = create(:user)
      ...
      mail = NotificationMailer.manager_notification_email(@user, @warning).deliver

      mail.subject.should eq(I18n.t("warnings.warning") + '!')
      mail.to.should eq([m1.email, m2.email, m3.email])
      mail.from.should eq(["admin@aasii.org"])
      mail.body.encoded.should match(I18n.t('warnings.limit.amount.title') + ": #{@user.entity.tag}")
      mail.body.encoded.should match(I18n.t('warnings.limit.amount.message',
                                            warning_object_id: @warning.object.id,
